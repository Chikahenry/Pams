...
export class SampleTemplateComponent implements OnInit {
  ...
  getsampletemplates() {
    ...
    this.apiUrl = environment.AUTHAPIURL + 'sampletemplates-list';
    ...
    this.httpClient.post<any>(this.apiUrl, obj, { headers: reqHeader }).subscribe(data => {
      ...
      this.sampletemplatesData = data.response == null ? [] : data.response.data;
  ...
  loadSelectedAssessmentData(selectedAssessment) {
    let sampletemplatestatus = selectedAssessment.status == 0 ? 'In Active' : 'Active';
    ...
    this.assessmentForm = this.formBuilder.group({
      ...
      sampletemplatestatus: [sampletemplatestatus],
  ...
  getSingleAssessment(assessmentId) {
    this.spinnerService.show();
    this.apiUrl = environment.AUTHAPIURL + 'sampletemplates/' + assessmentId;
